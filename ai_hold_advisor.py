...
_COOLDOWN_SEC = 60
_last_call = {}  # order_id → (timestamp, decision)
...
def ask_haiku_hold_or_close(order, spx_price, vix, trigger_reason, log_fn=None):
    ...
    # Cooldown check — don't spam Haiku
    now = time.time()
    if order_id in _last_call:
        last_time, last_decision = _last_call[order_id]
        if now - last_time < _COOLDOWN_SEC:
    ...
    try:
        ...
        is_call = ('CALL' in strategy or direction == 'BEARISH')
        is_put = ('PUT' in strategy or direction == 'BULLISH')
        ...
        # Parse short strike for distance calc
        short_strike = 0
        try:
        ...
        # Expiry is 4:00 PM ET = 20:00 UTC (during EDT) or 21:00 UTC (during EST)
        # Approximate: 6.5 trading hours from 9:30 AM
        import pytz
...
MARKET:
- SPX: {spx_price:.0f}
- Short strike: {short_strike:.0f} ({abs(distance_pts):.0f} pts {distance_label})
...
RECHECK: [Price level or time to reassess, e.g., "SPX 6740 or 11:00 AM"]"""
        ...
        _log(f"🧠 AI HOLD RESPONSE ({elapsed:.1f}s, ${response.usage.input_tokens * 0.80/1e6 + response.usage.output_tokens * 4.0/1e6:.4f}):")
        ...
        _last_call[order_id] = (time.time(), result)
        return result
