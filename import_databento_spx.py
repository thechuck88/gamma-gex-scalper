#!/usr/bin/env python3
"""
import_databento_spx.py — Import Databento SPX Options Data to Database
...
# Paths
CSV_FILE = Path(__file__).parent / "databento_data/OPRA-20260110-VYWACP8R4M/opra-pillar-20250109-20260108.ohlcv-1s.csv"
...
def clean_symbol(symbol):
    ...
    # "SPX   250815P04575000" → "SPX250815P04575000"
...
def get_database_stats():
    ...
    print(f"\n{'='*70}")
    print("DATABASE STATISTICS - ALL DATA")
    print(f"{'='*70}\n")
    ...
    query_spx = """
        SELECT
            COUNT(*) as spx_bars,
            COUNT(DISTINCT symbol) as spx_symbols,
            MIN(datetime) as spx_first,
            MAX(datetime) as spx_last
        FROM option_bars_1min
        WHERE symbol LIKE 'SPX%'
    """

    df_spx = pd.read_sql_query(query_spx, conn)

    print(f"\n{'='*70}")
    print("SPX OPTIONS ONLY")
    print(f"{'='*70}\n")

    print(df_spx.to_string(index=False))

    # Sample data
    print(f"\n{'='*70}")
    print("SAMPLE SPX DATA")
    print(f"{'='*70}\n")
...
def main():
    print(f"\n{'='*70}")
    print("DATABENTO SPX OPTIONS IMPORT")
    print(f"{'='*70}\n")

    # Check files exist
    if not CSV_FILE.exists():
        print(f"❌ CSV file not found: {CSV_FILE}")
        print("   Run: python3 download_databento_ndx.py --batch-id OPRA-20260110-VYWACP8R4M")
        sys.exit(1)
    ...
    # Read CSV
    print("Reading CSV file (this may take a few minutes)...")
    ...
    print("✅ SPX options data successfully imported!")
